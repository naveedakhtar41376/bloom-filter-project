...
The repository also includes `CONTRIBUTING.md` with commit-message and branch guidance.
 ## Insert time interpretation
This represents the relationship between the insertion time(seconds) and the number of inserted items. The inserted item is the words inserted into the bloom filter. As the number of inserted words increases, so does the time required for insertion of the words, which is consistent with the expected insertion complexity of the bloom filter for every item. For instance when n=1000, the insert time is approximately 0.0042 seconds, the insertion time per item is 4.23e-06 seconds, and the positive search time per item is 4.505e-06 seconds; when n=10,000, the insertion time is about 0.042 seconds, the insertion time per item is 4.23e-06 seconds, and the positive search time per item is 4.503e-06 seconds; when n=1,000,000, the insertion time is about 4.20 seconds, the insertion time per item is 4.22e-06 seconds, and the positive search time per item is 4.538e-06 seconds. From these values we can see that the insertion time per item is stable and remains almost identical. The scaling is almost linear in the number of inserted items because for a given configuration of a filter, the number of hash functions is fixed. The curvature in the graph is due to the fact that the number of inserted items is on logarithmic scale.
This plot shows that with an increasing  number of inserted words, the bloom filter scales efficiently because the  insertion time per item is approximately constant.This is consistent with the insertion complexity.

## search time interpretation
The graph demonstrates how search time evolves with the number of queried items. The y-axis represents search time in seconds, and the x-axis represents the number of queried items. It can be seen that as the number of queried items increases, so does the search time for both inserted(blue curve) and unseen items(orange curve). There is almost a linear relationship between the search time and the number of queried items. The curvatures in the graph is due to the fact that the values on the x-axis are on logarithmic scale. This sits well with the bloom filter’s theoretical complexity. For instance when n=1000, the positive search time is approximately 0.0045 seconds, and the negative search time is about 0.00391 seconds; when n=10,000, the positive search time is about 0.045 seconds, and the negative search time per item is 0.0393 seconds; when n=1,000,000, the positive search time is about 4.43 seconds, the negative search time is 3.90 seconds.
The graph displays that for the inserted items, the search time increases slightly faster than for unseen items. No clear distinction between the two items can be seen until around 10^6 queried items; there, the graph displays a distinct search time evolution between the inserted and unseen items, supporting the implementation’s correctness.
In conclusion, the figure is consistent with the theoretical concept of a Bloom filter. An increase in search time can be observed with an increase in the number of queried items. However, it remains resource-efficient for sets of queries. Since the negative queries can be terminated even before checking, we expect the search time for the unseen items to be slightly lower. This confirms the scalability of the implementation and shows behavior that is consistent with the expected lookup complexity for every item.

## hash distribution interpretation
The bar graph represents the hash-position distribution diagnostic. On the y-axis, we have the mean absolute relative bucket deviation, and on the x-axis, we have data type: words, dna, and random.
It is used to check if the items are evenly distributed for the above-mentioned data types by the hash functions. The mean absolute relative bucket deviation measures the deviation of the distribution of the observed hash position from an ideal uniform distribution. Higher value means less uniformity in the hash, and lower value means more uniform hash distribution. From the bars(words, DNA, and random), we observed that the values are all around 0.030 mean absolute relative deviation; they are almost identical. The BLAKE2b double-hashing distributes the values across all data types consistently, which means it is not biased towards a specific data type. 
