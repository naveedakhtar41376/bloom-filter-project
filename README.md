...
## Interpretation guide for final results

After the final HPC run, the conclusions should discuss:

1. Whether insertion time grows approximately linearly with the number of inserted items.
2. Whether search time grows approximately linearly with the number of queried items.
3. Whether empirical false-positive rates are close to theoretical false-positive rates near the designed capacity.
4. How strongly the false-positive rate increases when the filter receives more items than it was designed for.
5. How lowering the target false-positive rate increases Bloom filter memory usage.
6. How the Bloom filter memory compares with an exact Python set representation.
7. Whether hash-position diagnostics look similar for words, DNA strings, and random strings.
